		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				DECLARE PUBLICATIONCURSOR CURSOR LOCAL READ_ONLY FOR
					SELECT PUBLICATION, PUBLICATION_ID, PUBLICATION_TYPE, PUBLISHER_DB, ALLOW_QUEUED_TRAN, description
				...
				FETCH NEXT FROM PUBLICATIONCURSOR INTO @PUBLICATIONAME, @PUBLICATIONID, 
					@PUBLICATIONTYPE, @PUBLICATIONDATABASE, @ALLOW_QUEUED_TRAN, @DESCRIPTION
				WHILE @@FETCH_STATUS = 0
					BEGIN
						...
						SET @CONSOLEMSG = @CONSOLEMSG + ' - Description: ' + @DESCRIPTION
						RAISERROR (@CONSOLEMSG,10,1) WITH NOWAIT
						SET @CONSOLEMSG = 'XXX'
