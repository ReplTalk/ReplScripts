...
Modified: Taiyeb Zakir (tzakir@microsoft.com) -- added validation and Pull Subscription
...
--Modified for Pull
--Added login timeout
--redirection needed only for Pub Node1
--added validation
*/
...
:SETVAR SubDB "TESTPUBAGDB_Sub"
  ...
  --needed only if the nodes are in MultiSubnet
  /*
:CONNECT $(DisNode1)
 	--this will be applicable for all linked servers
...
(
WITH IP
((N'$(SubListenerIP1)', N'$(SubnetMask)')) --this is for one IP
--((N'$(SubListenerIP1)', N'$(SubnetMask)'),(N'$(SubListenerIP2)', N'$(SubnetMask)')) -- this for 2 IPs
...
@sync_type = N'Automatic', @subscription_type = N'pull', @update_mode = N'read only'
go
 
:Connect $(NodeSub1)
use $(SubDB);
exec sp_addpullsubscription 
@publisher = N'$(NodePub1)',
@publication = N'$(PubName)',
@publisher_db ='$(AGPubDatabase)',
@subscription_type = N'pull'
go
--if distributor is not specified then get this error when the Dist agent runs
--The server 'TR-VMAGPUB1' is not configured as a Distributor.
...
@distributor = '$(DisListenerName)', @distributor_security_mode = 1, @distributor_login = N'', @distributor_password = N''
GO 
...
print 'Validate redirected_publisher'
